...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    ...
    try:
        if subprocess.check_output(["pidof", "cryptolarid"]) is not None:
            print("%sWARNING!%s There is already a cryptolarid process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/cryptolarid' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/cryptolari-cli' + exeext
